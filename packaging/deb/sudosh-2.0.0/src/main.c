...
/* Global flag to track if we're in command-line mode */
...
/* Global sudo CLI option flags */
int sudo_askpass_mode = 0;          /* -A: use askpass helper */
int sudo_background_mode = 0;       /* -b: run in background */
int sudo_bell_mode = 0;             /* -B: ring bell when prompting */
int sudo_edit_mode = 0;             /* -e: edit files instead of running command */
int sudo_login_mode = 0;            /* -i: run login shell */
int sudo_reset_timestamp = 0;       /* -k: reset timestamp */
int sudo_remove_timestamp = 0;      /* -K: remove timestamp completely */
int sudo_non_interactive = 0;       /* -n: non-interactive mode */
int sudo_stdin_password = 0;        /* -S: read password from stdin */
int sudo_shell_mode = 0;            /* -s: run shell */
int sudo_validate_only = 0;         /* -v: validate user's timestamp */
char *sudo_chroot_dir = NULL;       /* -R: chroot directory */
int sudo_command_timeout = 0;       /* -T: command timeout in seconds */
...
static int execute_single_command(const char *command_str, const char *target_user) {
    ...
    if (test_env && strcmp(test_env, "1") == 0) {
        ...
        effective_user = (char *)(target_user ? target_user : "root");
    }
    ...
    if (!has_nopasswd) {
        /* Handle bell prompt if requested */
        handle_bell_prompt();

        /* Authenticate user based on selected method */
        int auth_result = 0;
        int auth_method = get_authentication_method();

        switch (auth_method) {
            case AUTH_METHOD_ASKPASS:
                auth_result = handle_askpass_authentication(username);
                break;
            case AUTH_METHOD_STDIN:
                auth_result = handle_stdin_password_authentication(username);
                break;
            default:
                auth_result = authenticate_user(username);
                break;
        }
    ...
    int validation_result = validate_command(command_str);
    if (validation_result == 0) {
        ...
        /* Shell restriction triggered - fall back to interactive mode */
        free_user_info(user);
        free(username);
        command_line_mode = 0; /* Reset flag for interactive mode */
        return main_loop(); /* Start interactive mode */
    }
    ...
    if (sudo_chroot_dir && !apply_chroot()) {
        fprintf(stderr, "sudosh: failed to apply chroot\n");
...
int main_loop(void) {
    ...
    while (!is_interrupted()) {
        ...
        /* Validate command for security */
        int validation_result = validate_command(command_line);
        if (validation_result == 0) {
            fprintf(stderr, "sudosh: command rejected for security reasons\n");
            free(command_line);
            continue;
        } else if (validation_result == -2) {
            /* Shell restriction triggered in interactive mode - just continue to prompt */
            free(command_line);
            continue;
        }
...
int main(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            ...
            printf("Options:\n");
            printf("  -A                      Use askpass helper for password prompting\n");
            printf("  -b                      Run command in background\n");
            printf("  -B                      Ring bell when prompting for password\n");
            printf("  -e                      Edit files instead of running command\n");
            printf("  -h, --help              Show this help message\n");
            printf("  -i                      Run login shell as target user\n");
            printf("  -k                      Reset user's timestamp (invalidate cached credentials)\n");
            printf("  -K                      Remove user's timestamp file completely\n");
            printf("  -l, --list              List available commands showing each permission source\n");
            printf("  -L, --log-session FILE  Log entire session to FILE\n");
            printf("  -n                      Non-interactive mode (no prompts)\n");
            printf("  -R DIR                  Change root directory to DIR before executing command\n");
            printf("  -s                      Run shell as target user\n");
            printf("  -S                      Read password from standard input\n");
            printf("  -T TIMEOUT              Set command timeout in seconds\n");
            printf("  -u, --user USER         Run commands as target USER\n");
            printf("  -v                      Validate user's timestamp without running command\n");
            ...
            /* Check if this is actually the validate option */
            if (i + 1 < argc && argv[i + 1][0] != '-') {
                /* This is verbose mode with a command following */
                verbose_mode = 1;
            } else if (i + 1 >= argc || argv[i + 1][0] == '-') {
                /* This is validate mode (-v without arguments) */
                sudo_validate_only = 1;
                verbose_mode = 1;  /* Also enable verbose for validation */
            } else {
                verbose_mode = 1;
            }
            ...
            sudo_stdin_password = 1;
        } else if (strcmp(argv[i], "-k") == 0) {
            sudo_reset_timestamp = 1;
        } else if (strcmp(argv[i], "-K") == 0) {
            sudo_remove_timestamp = 1;
        } else if (strcmp(argv[i], "-n") == 0) {
            sudo_non_interactive = 1;
        } else if (strcmp(argv[i], "-b") == 0) {
            sudo_background_mode = 1;
            ...
            if (i + 1 >= argc) {
                fprintf(stderr, "sudosh: option -R requires an argument\n");
                return EXIT_FAILURE;
            }
            if (!handle_chroot_option(argv[++i])) {
                return EXIT_FAILURE;
            }
        } else if (strcmp(argv[i], "-T") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "sudosh: option -T requires an argument\n");
                return EXIT_FAILURE;
            }
            sudo_command_timeout = atoi(argv[++i]);
            if (sudo_command_timeout <= 0) {
                fprintf(stderr, "sudosh: invalid timeout value: %s\n", argv[i]);
                return EXIT_FAILURE;
            }
            ...
            /* Non-option argument: treat as command to execute or files to edit */
            if (sudo_edit_mode) {
                /* Edit mode: collect files to edit */
                char **files = &argv[i];
                int file_count = argc - i;

                /* Validate edit mode */
                if (!handle_edit_mode(files, file_count)) {
                    return EXIT_FAILURE;
                }

                /* Execute edit command */
                execute_edit_command(files, file_count, target_user);
                /* execute_edit_command only returns on failure (execv doesn't return on success) */
                return EXIT_FAILURE;
            } else {
                /* Command mode: build command from remaining arguments */
    ...
    if (!validate_sudo_option_combinations()) {
        return EXIT_FAILURE;
    }

    /* Check if edit mode was specified but no files provided */
    if (sudo_edit_mode) {
        fprintf(stderr, "sudosh: no files specified for editing\n");
        fprintf(stderr, "Usage: sudosh -e file1 [file2 ...]\n");
        return EXIT_FAILURE;
    }
    ...
    /* Handle timestamp management options first */
    if (sudo_reset_timestamp) {
        int result = handle_reset_timestamp(username);
        free(username);
        return result ? EXIT_SUCCESS : EXIT_FAILURE;
    }

    if (sudo_remove_timestamp) {
        int result = handle_remove_timestamp(username);
        free(username);
        return result ? EXIT_SUCCESS : EXIT_FAILURE;
    }

    if (sudo_validate_only) {
        int result = handle_validate_timestamp(username);
        free(username);
        return result ? EXIT_SUCCESS : EXIT_FAILURE;
    }
    ...
    /* Set up command timeout if specified */
    if (sudo_command_timeout > 0) {
        setup_command_timeout(sudo_command_timeout);
    }
