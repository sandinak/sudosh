...
char *expand_equals_expression(const char *arg) {
    if (!arg || arg[0] != '=') {
        return sudosh_safe_strdup(arg);  /* Return copy of original if not = expression */
    }

    /* Skip the = character */
    const char *command_name = arg + 1;
    if (strlen(command_name) == 0) {
        return sudosh_safe_strdup(arg);  /* Return original if just = */
    }

    /* Get PATH environment variable */
    char *path_env = getenv("PATH");
    if (!path_env) {
        return sudosh_safe_strdup(arg);  /* Return original if no PATH */
    }

    /* Make a copy of PATH for tokenization */
    char *path_copy = sudosh_safe_strdup(path_env);
    if (!path_copy) {
        return sudosh_safe_strdup(arg);
    }
    ...
    while (dir != NULL) {
        ...
        if (stat(full_path, &st) == 0 && S_ISREG(st.st_mode) && (st.st_mode & S_IXUSR)) {
            ...
            result = sudosh_safe_strdup(full_path);
            break;
        }
    ...
    return result ? result : sudosh_safe_strdup(arg);
}
...
int parse_command(const char *input, struct command_info *cmd) {
    ...
    input_copy = sudosh_safe_strdup(input);
    if (!input_copy) {
        SUDOSH_LOG_ERROR("Failed to duplicate command input");
        return -1;
    }

    /* Allocate initial argv array */
    cmd->argv = sudosh_safe_malloc(argv_size * sizeof(char *));
    if (!cmd->argv) {
        free(input_copy);
        return -1;
    }
    ...
    cmd->command = sudosh_safe_strdup(input);
    if (!cmd->command) {
        free_command_info(cmd);
        free(input_copy);
        return -1;
...
int validate_ansible_command(const char *command, const char *username) {
    ...
    if (global_ansible_info && global_ansible_info->is_ansible_session) {
        ...
        if (!cmd_copy) {
            SUDOSH_LOG_ERROR("Failed to duplicate command for Ansible validation");
            return 0;
        }
...
int execute_command(struct command_info *cmd, struct user_info *user) {
    ...
    if (cmd->argv[0][0] != '/') {
        ...
        command_path = sudosh_safe_strdup(cmd->argv[0]);
        if (!command_path) {
            SUDOSH_LOG_ERROR("Failed to duplicate command path");
            return -1;
        }
    }
...
char *find_command_in_path(const char *command) {
    ...
    path_copy = sudosh_safe_strdup(path_env);
    if (!path_copy) {
        SUDOSH_LOG_ERROR("Failed to duplicate PATH for command search");
        return NULL;
    }
    ...
    while (dir != NULL) {
        ...
        full_path = sudosh_safe_malloc(path_len);
        if (!full_path) {
            free(path_copy);
            return NULL;
        }
