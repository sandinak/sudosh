...
int test_validate_safe_redirection() {
    ...
    /* Tilde expansion should be allowed only when it maps to HOME and HOME is used */
    {
        const char *old_home = getenv("HOME");
        setenv("HOME", "/root", 1);
        TEST_ASSERT_EQ(1, is_safe_redirection_target("~/ok.txt"),
                       "~ inside HOME should be safe (even if HOME=/root)");
        TEST_ASSERT_EQ(0, is_safe_redirection_target("/root/bad.txt"),
                       "Absolute /root should remain blocked");
